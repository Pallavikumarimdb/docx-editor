...
- **Fonts:** Custom and embedded fonts referenced in the DOCX must render. If the editor has a font-loading config option, use it. If fonts are embedded in the DOCX zip, they must not be dropped.
...
1. **Display a DOCX** — load a `.docx` file and render it with full formatting fidelity using the editor.
