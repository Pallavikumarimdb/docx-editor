...
import { paragraphToRuns } from '../../../flow-model/buildBoxTree/runs';
...
describe('Issue #728 & #727 — Hebrew/RTL text complex script styling and round-trip preservation', () => {
  ...
  test('flow-model resolves fontSize to fontSizeCs for RTL runs', () => {
  ...
  test('flow-model resolves fontSize to fontSizeCs for RTL runs when normal size is absent', () => {
  ...
  test('flow-model resolves fontSize to normal size for non-RTL runs', () => {
    const fontSizeMark = schema.marks.fontSize.create({ size: 24, sizeCs: 36 });
  ...
  test('flow-model resolves fontFamily to cs for RTL runs', () => {
  ...
  test('flow-model resolves fontFamily to ascii for non-RTL runs', () => {
    const fontFamilyMark = schema.marks.fontFamily.create({ ascii: 'Arial', cs: 'David' });
