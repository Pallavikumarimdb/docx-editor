...
export class ClickPositionResolver {
  ...
  #indexRunsInFragment(fragmentEl: HTMLElement, blockId: string): void {
    const runs: RunInfo[] = [];
    const runEls = fragmentEl.querySelectorAll('span[data-doc-from][data-doc-to]');

    for (const runEl of Array.from(runEls)) {
      const element = runEl as HTMLElement;
      const docFrom = Number(element.dataset.docFrom);
      const docTo = Number(element.dataset.docTo);
      const rect = element.getBoundingClientRect();
  ...
  getPositionInRun(run: RunInfo, clientX: number): number {
    ...
    if (run.isTab) {
      ...
      return clientX < midpoint ? run.docFrom : run.docTo;
    }
    ...
    if (!ownerDoc) return run.docFrom;
    ...
    const charIndex = Math.max(0, Math.min(left, run.docTo - run.docFrom));
    return run.docFrom + charIndex;
  }
  ...
  getPositionAtPoint(clientX: number, clientY: number): PositionLookupResult | null {
    ...
    if (!run) {
      ...
      const docFrom = Number(fragment.element.dataset.docFrom);
      return {
        pageIndex: page.index,
        pmPosition: docFrom || 0,
  ...
  getElementAtPosition(pmPos: number): HTMLElement | null {
    ...
    for (const runs of this.#runsByFragment.values()) {
      for (const run of runs) {
        ...
        if (run.isTab) {
          if (pmPos >= run.docFrom && pmPos < run.docTo) {
            return run.element;
          }
        } else {
          if (pmPos >= run.docFrom && pmPos <= run.docTo) {
