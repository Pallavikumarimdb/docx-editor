...
function cssBorderKindToOoxml(cssStyle: string): BorderSpec['style'] {
...
function readBorderAttrsFromCSS(style: CSSStyleDeclaration): TableCellAttrs['borders'] | null {
  const parseSide = (
    cssStyle: string,
    cssColor: string,
    cssWidth: string
    ...
    return {
      style: cssBorderKindToOoxml(cssStyle),
...
function parseCellAttrsFromDOM(element: HTMLTableCellElement): TableCellAttrs {
  ...
  const borders = readBorderAttrsFromCSS(style);
...
function buildCellBorderKinds(attrs: TableCellAttrs): string[] {
  const styles: string[] = [];
...
const tableCellSpec: NodeSpec = {
  ...
  toDOM(node) {
    ...
    styles.push(...buildCellBorderKinds(attrs));
      ...
      return (state, dispatch) => {
        ...
        if (dispatch) {
          ...
          for (const { pos } of targetCells) {
            ...
            let cellEdgeSet: Record<string, typeof solidBorder | typeof noBorder>;
            switch (preset) {
              case 'all':
                cellEdgeSet = {
                  top: solidBorder,
              ...
              case 'inside':
                ...
                break;
              case 'none':
                cellEdgeSet = { top: noBorder, bottom: noBorder, left: noBorder, right: noBorder };
                break;
            }
