 ...
 *
 * Both the painter (`renderParagraph.ts`) and the measurer (`paragraphLayout`)
...
import type { ParagraphBlock, TextRun } from '../../pagination-model/types';
 ...
 *    cursor).
 *  - `w:defaultTabMark` (§17.6.13): default-grid stops at multiples of the
 ...
 *
 * `attrs.tabs` here is the pagination `TabMark` shape (`val` + `pos`),
 * not the docx-types `TabMark` (`alignment` + `position`).
 */
