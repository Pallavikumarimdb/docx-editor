...
/**
 * Serialize a simple field back as `<w:fldSimple>` so the source form is
 * preserved on round-trip (complex fields stay complex — see
...
export function serializeSimpleField(field: SimpleField): string {
  ...
  if (!contentXml) return `<w:fldSimple ${attrs.join(' ')}/>`;
