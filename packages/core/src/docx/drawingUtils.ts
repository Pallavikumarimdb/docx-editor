...
import { getThemeColor } from './themeParser';
...
function getCanonicalThemeColorSlot(
  slot: NonNullable<ColorValue['themeColor']>
): keyof ThemeColorScheme {
  switch (slot) {
    case 'text1':
      return 'dk1';
    case 'text2':
...
export function resolveColorValueToHex(color: ColorValue | undefined): string | undefined {
  ...
  if (color.themeColor) {
    return `#${getThemeColor(undefined, getCanonicalThemeColorSlot(color.themeColor))}`;
  }
