 ...
 */
function parseTabMarks(tabs: XmlElement | null): TabMark[] | undefined {
...
export function parseParagraphProperties(
  ...
  if (spacing) {
    ...
    // See ParagraphFormatting.spacingOverrides.
    ...
    if (explicit.before || explicit.after) {
      formatting.spacingOverrides = explicit;
    }
  ...
  if (tabs) {
    formatting.tabs = parseTabMarks(tabs);
  }
