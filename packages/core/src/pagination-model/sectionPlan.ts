 ...
 * know are wrong until block 200.
 *
 * So we don't. One pass up front turns the breaks into an immutable schedule,
 * and the flow stage only ever *looks up* the geometry in force at a block.
 * A page's size is a lookup, never the product of accumulated mutations.
 *
 * @packageDocumentation
 */

import type { FlowBlock, SectionLayoutConfig } from './types';

/**
 * Which geometry governs which blocks.
 *
 * `configs[k]` is the geometry of section `k`. `breakIndices[k]` is the block
 * index of the `sectionBreak` that *ends* section `k` — so section `k` covers
 * the blocks up to and including that break, and there is always one more
...
/**
 * Resolve the section breaks in `blocks` into a schedule.
 *
 * A `sectionBreak` block carries the properties of the section it *closes*, not
...
export function collectSectionConfigs(
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const config: SectionLayoutConfig = {
      pageSize: block.pageSize ?? inherited.pageSize,
      margins: block.margins ?? inherited.margins,
      columns: block.columns,
      startType: block.type,
    };
