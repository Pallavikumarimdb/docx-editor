...
export function layoutColumns(
  ...
  const colConfig = columnConfigForuration(sectionProps, contentWidthPx);
 ...
 */
function columnConfigForuration(
  sectionProps: SectionProperties,
  contentWidthPx: number
): ColumnConfig {
...
function layoutParagraphInColumn(
  ...
  const lineResult = breakIntoLines(paragraph, {
    ...
    firstLineIndent: paragraph.formatting?.indentation?.firstLine
      ...
      : 0,
    tabMarks: paragraph.formatting?.tabs,
    theme,
...
export function getColumnWidths(
  ...
  const config = columnConfigForuration(sectionProps, contentWidthPx);
