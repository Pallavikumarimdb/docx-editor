 ...
 */
export const BORDER_STYLES: { value: BorderKindType; label: string }[] = [
...
export function TableBorderPicker({
  ...
  const handleStyleChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    setBorderConfig((prev) => ({ ...prev, style: e.target.value as BorderKindType }));
  }, []);
 ...
 */
export function mapStyleToCss(style: BorderKindType): string {
  const mapping: Record<BorderKindType, string> = {
    none: 'none',
    single: 'solid',
    double: 'double',
    dotted: 'dotted',
...
export function createBorderConfig(spec: BorderSpec | undefined): BorderConfig {
  ...
  return {
    style: (spec.style as BorderKindType) || 'single',
 ...
 */
export function getAvailableBorderKinds(): typeof BORDER_STYLES {
