...
import type { TabMark, TabLeader, TabJustify } from '../../types/document';
...
export function calculateTabRenderInfo(
  ...
  const widthTwips = calculateTabWidth(currentPosition, tabMarks, pageWidth);
  ...
  return {
    ...
    tabMarkPosition: nextTab.position,
    hasLeader,
    leaderString,
  };
}
...
export function Tab({
  ...
  // Calculate tab rendering information
  const info = calculateTabRenderInfo(currentPosition, tabMarks, pageWidth);
  ...
  return (
    <span
      ...
      style={combinedStyle}
      data-tab-position={info.tabMarkPosition}
