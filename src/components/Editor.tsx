...
export interface EditorProps {
  ...
  paintPage?: (content: ReactNode, pageIndex: number, sectionProps: SectionProperties) => ReactNode;
...
export const Editor = React.forwardRef<EditorRef, EditorProps>(function Editor(
   ...
   */
  const paintPageContent = useCallback(
    (page: PageData) => {
  ...
  const renderSinglePage = useCallback(
    (page: PageData, totalPages: number) => {
      ...
      const pageContent = paintPageContent(page);

      // Use custom page renderer if provided
      if (paintPage) {
        return paintPage(pageContent, page.pageNumber - 1, page.sectionProps);
      }
    ...
    [zoom, showPageShadows, showMarginGuides, marginGuideColor, paintPageContent, paintPage, theme]
  );
  ...
  const renderedPages = useMemo(() => {
    ...
    const renderedPage = paintPage
      ? paintPage(pageContent, 0, sectionProps)
      : (
        <div
          className="docx-editor-page"
  ...
  }, [enablePagination, pageLayout, renderAllContent, paintPage, sectionProps, zoom, showPageShadows, showPageNumbers, showMarginGuides, marginGuideColor, pageGap]);
