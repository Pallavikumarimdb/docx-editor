 ...
 */
export function getMergedSelectionGeometry(
  containerElement?: HTMLElement | null
): HighlightRect[] {
  const rects = getSelectionGeometry(containerElement);
...
export function createSelectionChangeHandler(
  ...
  return () => {
    ...
    const rects = merge
      ? getMergedSelectionGeometry(containerElement)
      : getSelectionGeometry(containerElement);
