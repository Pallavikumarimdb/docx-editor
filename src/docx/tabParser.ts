 ...
 */
export const DEFAULT_TAB_ALIGNMENT: TabJustify = 'left';
 ...
 * @returns Parsed TabMark or null if invalid
 */
export function parseTabMark(tab: XmlElement): TabMark | null {
 ...
 * @returns Array of TabMark objects, sorted by position
 */
export function parseTabMarks(tabs: XmlElement | null): TabMark[] {
  if (!tabs) return [];

  const tabElements = findChildren(tabs, 'w', 'tab');
  if (tabElements.length === 0) return [];

  const result: TabMark[] = [];

  for (const tab of tabElements) {
    const tabMark = parseTabMark(tab);
    if (tabMark) {
      result.push(tabMark);
    }
  }
 ...
 * @returns Array of TabMark objects or undefined if none
 */
export function parseTabMarksFromParagraphProperties(
  pPr: XmlElement | null
): TabMark[] | undefined {
  if (!pPr) return undefined;

  const tabs = findChild(pPr, 'w', 'tabs');
  const tabMarks = parseTabMarks(tabs);

  return tabMarks.length > 0 ? tabMarks : undefined;
}
 ...
 */
export function mergeTabMarks(
  styleTabs: TabMark[] | undefined,
  directTabs: TabMark[] | undefined
): TabMark[] {
  ...
  const tabMap = new Map<number, TabMark>();
 ...
 * @param tabMarks - Array of tab stops
 * @param tolerance - Position tolerance in twips (default 10)
 * @returns TabMark at that position or undefined
 */
export function findTabMarkAtPosition(
  position: number,
  tabMarks: TabMark[],
  tolerance: number = 10
): TabMark | undefined {
  return tabMarks.find(
    (tab) =>
...
export function calculateTabWidth(
  ...
  const nextTab = getNextTabMark(currentPosition, tabMarks, pageWidth);
...
): { width: number; alignment: TabJustify } {
  const nextTab = getNextTabMark(currentPosition, tabMarks, pageWidth);
  let width: number;
 ...
 */
export function isValidTabJustify(value: string): value is TabJustify {
  const validAlignments: TabJustify[] = [
    'left', 'center', 'right', 'decimal', 'bar', 'clear', 'num',
  ];
  return validAlignments.includes(value as TabJustify);
}
 ...
 */
export function generateDefaultTabMarks(
  pageWidth: number,
  interval: number = DEFAULT_TAB_INTERVAL_TWIPS
): TabMark[] {
  const tabMarks: TabMark[] = [];
 ...
 */
export function getEffectiveTabMarks(
  explicitTabs: TabMark[] | undefined,
  pageWidth: number
): TabMark[] {
  if (!explicitTabs || explicitTabs.length === 0) {
    return generateDefaultTabMarks(pageWidth);
  }
